...
namespace IPair {
    ...
    TPair::TPair(const TPair& it) {
        value = it.value;
        memcpy(key, it.key, MAX_KEY_LENGTH);
    }
...
namespace IBTree {
    ...
    void TBTree::SaveNode3(TBTreeNode* curNode, std::ofstream &file) {
        size_t size = curNode->pairs.size();
        for (unsigned i = 0; i < size; ++i) {
            file.write((char *)(&curNode->pairs[i].key), MAX_KEY_LENGTH*sizeof(char));
            file.write((char *)(&curNode->pairs[i].value), sizeof(unsigned long long));
        }

        size_t sizeChild = curNode->children.size();
        if (curNode->children[0] != nullptr) {
            for (unsigned i = 0; i < sizeChild; ++i) {
    ...
    void TBTree::Load3(std::ifstream& file) {
        ...
        for (unsigned i = 0; i < size; ++i) {
            TPair tempPair;
            file.read((char *)(&tempPair.key), MAX_KEY_LENGTH*sizeof(char));
            file.read((char *)(&tempPair.value), sizeof(unsigned long long));
            this->Push(tempPair);
        }
    }
...
int main() {
    ...
    auto toLower = [](char buf[MAX_KEY_LENGTH], TPair& curPair) 
    {
        bool toClean = false;
        for (unsigned i = 0; i < MAX_KEY_LENGTH; ++i) {
            if (toClean == true) {
    ...
    while (std::cin >> command) 
    {
        ...
        {
            strncpy(buf, command.c_str(), command.length());
            buf[command.length()] = '\0';
            toLower(buf, curPair);
            curSearchNode = nullptr;
            curSearchPosition = 0;
            btree.Find(curSearchNode, curSearchPosition, curPair);
