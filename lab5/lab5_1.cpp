...
struct SuffixTree {
    struct Edge {
        ...
        Edge(int left, std::shared_ptr<int> right, int node): 
            left(left), 
            right(right), 
            nodeTarget(node) {}
    };
...
int main() {
    ...
    std::cin >> text;
    std::vector<size_t> ms(text.size());
...
artemznaj@MacBook-Air-Artem lab5 % ./wrapper.sh
[info][Mon Sep 3 22:29:56 MSK 2022] Stage #1. Compiling...
